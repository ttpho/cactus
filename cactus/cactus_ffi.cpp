...
int cactus_tts_c(
    ...
    try {
        auto* ctx = static_cast<cactus_context*>(handle);
        
        // Process the text and generate audio
        std::vector<float> audio_data;
        int sample_rate;
        
        bool success = ctx->tts(
            params->text,
            params->speaker_file_path,
            params->n_threads,
            audio_data,
            sample_rate,
        ...
        // Allocate and copy the audio data
        result->audio_data = new float[audio_data.size()];
        std::copy(audio_data.begin(), audio_data.end(), result->audio_data);
        result->sample_count = audio_data.size();
        result->sample_rate = sample_rate;

        return 0;
    } catch (const std::exception& e) {
        LOG_ERROR("TTS error: %s", e.what());
        return -1;
    }
}

int cactus_save_wav_c(
    const float* audio_data,
    int32_t sample_count,
    int32_t sample_rate,
    const char* file_path
) {
    if (!audio_data || sample_count <= 0 || sample_rate <= 0 || !file_path) {
        return -1;
    }

    try {
        std::vector<float> audio_vec(audio_data, audio_data + sample_count);
        bool success = cactus::tts_internal::save_wav16(file_path, audio_vec, sample_rate);
        return success ? 0 : -1;
    } catch (const std::exception& e) {
        LOG_ERROR("Save WAV error: %s", e.what());
        return -1;
    }
}

void cactus_free_audio_data_c(cactus_tts_result_c_t* result) {
